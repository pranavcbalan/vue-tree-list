<template>
    <div class='vtl'>
        ...
        <span class="vtl-caret vtl-is-small" v-if="model.children && model.children.length > 0">
          ...
          <span title="add tree node" @click.stop.prevent="addChild(false)"
                v-if="!model.isLeaf && !model.addTreeNodeDisabled">
            <slot name="addTreeNode">
              <i class="vtl-icon vtl-icon-folder-plus-e"></i>
            </slot>
          </span>
                    <span title="add leaf node" @click.stop.prevent="addChild(true)"
            ...
            <slot name="delNode">
              <i class="vtl-icon vtl-icon-trash"></i>
                    ...
                    <span title="run" @click.stop.prevent="runNode" v-if="model.isLeaf && !model.runNodeDisabled">
            <slot name="runNode"></slot>
          </span>
                    <span>
            <slot name="extraLeafNode" v-if="model.isLeaf"></slot>
            <slot name="extraFolderNode" v-else></slot>
          </span>
        ...
        <div :class="{'vtl-tree-margin': model.name !== 'root'}" v-show="model.name === 'root' || expanded"
             v-if="isFolder">
            <item v-for="model1 in model.children"
...
<script>
  ...
  export default {
    ...
    computed: {
      itemProp () {
        const { parent, children, ...rest } = this.model
        return rest
      },
      itemIconClass () {
